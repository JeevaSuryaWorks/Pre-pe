...
export const WhatsAppBanner = () => {
    const [banners, setBanners] = useState<Banner[]>(() => {
        try {
            const cached = localStorage.getItem('prepe_banners_cached');
            if (cached) {
    ...
    const fetchBanners = async () => {
        try {
            ...
            if (data && data.length > 0) {
                setBanners(data);
                localStorage.setItem('prepe_banners_cached', JSON.stringify(data));
                setActive(0);
            } else {
                setBanners(FALLBACK);
            }
            ...
            // On fetch failure, keep cached banners if we have them instead of forcing FALLBACK
        }
    };
                        ...
                        <div className="relative z-10 p-5 flex items-center gap-4">
                            <div className="flex-1 min-w-0">
                                ...
                                {b.cta_text && (
                                    <Button
                                        size="sm"
                                        className="rounded-xl px-6 h-9 text-[11px] font-black border-none bg-white text-slate-900 shadow-xl hover:bg-slate-50 transition-all hover:scale-105 active:scale-95 uppercase tracking-widest mt-1"
                                    >
                                        {b.cta_text}
                                    </Button>
                                )}
